...
RSpec.describe Dotenv, :ktg, :printX do
  describe 'load' do
    ...
    context 'with simple key/values',
            ...
            content: [{
              label: 'Source',
              description: uc_add_content(src_dotenv, lines: [5, *(12..19), 85], code_type: 'ruby')
            }] do
  ...
  #{uc_add_content(fixture_path('dotenv', 'simple.env'), lines: [0, 1], code_type: 'bash')}
      " do
        subject
  ...
  #{uc_add_content(fixture_path('dotenv', 'simple.env'), lines: [*(3..5)], code_type: 'bash')}" do
        subject
    ...
    context 'with multiline values',
             ...
             content: [{
               label: 'Example',
               description: uc_add_content(fixture_path('dotenv', 'multiline.env'), code_type: 'bash')
             }] do
    ...
    context 'with quoted values',
            ...
            content: [{
              label: 'Example',
              description: uc_add_content(fixture_path('dotenv', 'quoted.env'), code_type: 'bash')
            }, {
              label: 'Output',
    ...
    context 'with YAML file input',
            usage: "#{described_class.name}.load('yaml.env')",
            content: [{
              label: 'Example',
              description: uc_add_content(fixture_path('dotenv', 'yaml.env'), code_type: 'yaml')
            }, {
              label: 'Output',
